...
* **OPEN — absent and zero are still indistinguishable on the sacrifice axis.** This is the unresolved design question the above exposes. An agent that never *reports* token usage and an agent that genuinely did no work both land on `sacrifice = 0`, and under a geometric mean both score 0.0. "Not scoreable" is arguably the honest third value, but the protocol has no representation for it today, and the AIS contract (§4.3) has no opinion. Deliberately not papered over with a default.
...
* **OPEN — vault anchoring silently stopped, and the honest-logging design did not catch it.** 10 commit leaves exist; `anchors.jsonl` records one anchor at `leaves_through: 1`; **9 leaves have been pending since 2026-07-30T08:42Z**. More telling, `session.log` contains exactly **one** `vault:` line in its entire history, though `session_stop.py` logs on *every* branch — the 14:01Z session-end logged its telemetry and then nothing. Reading the vault is not the problem (`session_root()` measured at 0.035s). The hook is being killed before or during the chain write, producing no record at all. Mitigated (not fixed) by logging a breadcrumb *before* the attempt, so a truncated run is visible as "entered, never finished". The real fix — moving the Base Sepolia write off the hook's critical path — is not built.

* **OPEN — all anchored evidence is empty.** Every one of the 10 leaves carries `test_result_hash: "unverified"` (4) or `"unverified:stale"` (6). Memory is being committed; what it commits attests to nothing.
