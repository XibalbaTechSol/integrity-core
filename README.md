...
|---|---|---|---|
| [`contracts/`](contracts/) | Solidity + Foundry | The 7 primitives, factory, registries, XNS, `IntegrityGovernance`, $ITK, Integrity Health stack, ZK verifier, cross-chain reputation bridge, local `IntegrityIdentityReadV1` facade, and an experimental non-deployed Phase I kernel slice (see "Whitepaper v3.2 implementation status" above) | ✅ 250 tests locally as of 2026-08-18 (re-verify with `forge test` before quoting — this number moves); the existing Base Sepolia deployment predates the identity facade and still has the older verifier bytecode (XNS/governance/CCIP bridge also not yet broadcast); the Phase I kernel slice is Foundry-test-only and not part of any deployment |
