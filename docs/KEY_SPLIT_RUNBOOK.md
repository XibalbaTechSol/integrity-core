...
**You can keep using one key for development.** Local Anvil, Foundry tests, scripts,
`.env`, and **Base Sepolia** MAY map every seat
(`arbitrator`, `disputer`, `funderWallet`, `governance`, `oracleSigner`, `resolverSigner`,
`MINTER_ROLE`, `DEFAULT_ADMIN_ROLE`) to a single EOA.

That is the current `deployments.baseSepolia.json` shape
(`0x67bA5D723E1F5517afF7eb980E2f73a9e17aD556`). It is an accepted **testnet** posture.
Nothing in this runbook, `MediationOk`, or CI should block a single-key Sepolia or local deploy.

Use one key when you are:
- iterating on contracts or the oracle locally
- running `forge test` / `forge script`
- operating Base Sepolia agents

Do not paste that key into chat, git, or CI logs. A leaked **testnet** key is still worth rotating (`docs/signer-role-rotation-2026-08.md`), but it is not a mainnet-incident class event.

## Mainnet posture (required)

The eight-key split is **P0 for mainnet only**. It is non-conformant as a mainnet posture.
Required before any mainnet `SovereignAgent` proxy.

Sequence (consequence order): distinct keys → process-held signers → constrain $ITK$ → then multisig/timelock.
...
## Roles (eight seats, eight keys) — mainnet
...
## Step 1 — Distinct keys (mainnet; optional early on Sepolia)
...
On mainnet, `oracleSigner` and `resolverSigner` MUST be keys held by the oracle / BCC middleware
processes, not a human wallet. On Sepolia, the funder EOA MAY still sign those roles.
...
Do not claim decentralized control before this lands. Testnet minting from the single operator key is fine.

## Step 4 — Safe / timelock (mainnet last)

Only after steps 1–3, and only for mainnet:
...
## Mainnet checklist (copy into the mainnet deploy PR)
...
Skip this checklist for Anvil and Base Sepolia.
