...
_RULES: List[Tuple[str, Pattern[str]]] = [
    ...
    ("SECRET_VALUE", re.compile(r"\b(?:password|passwd|passphrase|client_secret|access_token|refresh_token|secret|api[_ -]?key|database_url|db[_ -]?password|smtp[_ -]?(?:password|user)|aws[_ -]?secret|cloud[_ -]?(?:key|secret))\s*[:=]\s*[\"']?[^\s,;\"'}]+", re.IGNORECASE)),
    ("COOKIE", re.compile(r"\b(?:Cookie|Set-Cookie):\s*[^;\r\n]+", re.IGNORECASE)),
    ("RECOVERY_PHRASE", re.compile(r"\b(?:seed|recovery)\s+phrase\s*[:=]\s*(?:[A-Za-z]+\s+){11,23}[A-Za-z]+", re.IGNORECASE)),
