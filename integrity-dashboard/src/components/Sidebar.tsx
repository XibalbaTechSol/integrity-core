...
export function Sidebar() {
  ...
  const isActive = (item: (typeof NAVIGATION_ITEMS)[number]) =>
    ...
    <aside className="protocol-sidebar" >
      ...
      <nav aria-label="Protocol navigation" tabIndex={0}>
        {nav.map(item => {
          ...
          return <Link key={item.to} aria-label={item.label} className={isActive(item) ? 'active' : ''} to={item.to}><Icon size={17} /><span>{item.label}</span></Link>;
        })}
      </nav>
        ...
        <div className={`sidebar-profile ${profileOpen ? 'open' : ''}`} onClick={() => setProfileOpen(!profileOpen)}>
          ...
          <div className="profile-info">
            ...
            <small>{user ? 'Authenticated' : 'Sign-in required'}</small>
          </div>

          {profileOpen && (
            <div className="profile-dropdown">
              <Link to="/settings" onClick={(e) => e.stopPropagation()}>Settings</Link>
