...
def test_intercept_aos_gating(client, real_opa_server, monkeypatch):
    ...
    # 1. Deny when trace_id/span_id/agent_thought are missing on agent tool call
    ...
    # 2. Allow when trace_id/span_id/agent_thought are present
    payload_valid = sign_commitment(private_key, agent_id=agent_id, intent_type="claude_tool:Bash", nonce=2)
    payload_valid["trace_id"] = "4bf92f3577b34da6a3ce929d0e0e4736"
    payload_valid["span_id"] = "00f067aa0ba902b7"
    payload_valid["agent_thought"] = "Verifiable reasoning monologue explaining the action."
